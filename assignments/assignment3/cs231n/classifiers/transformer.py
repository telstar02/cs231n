...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # 1)
        out_features = self.visual_projection(features) # (N,D)->(N,W)
        out_features = out_features.unsqueeze(1) # (N,1,W)
        captions_embedded = self.embedding(captions) # (N,T)->(N,T,W)
        captions_embedded_pos = self.positional_encoding(captions_embedded)
        # 2)
        tgt_mask = torch.tril(torch.ones((T,T)).bool())
        # 3)
        scores = self.transformer(captions_embedded_pos, out_features, tgt_mask)
