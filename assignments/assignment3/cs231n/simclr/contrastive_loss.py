import torch
import numpy as np
from torchvision.datasets.utils import tarfile
...
def sim(z_i, z_j):
    ...
    ##############################################################################
    # 1×1 tensor of dot products
    numerator = torch.matmul(z_i, z_j.t())
    # scalar norms
    norm_i = torch.linalg.norm(z_i)
    norm_j = torch.linalg.norm(z_j)
    # elementwise divide numerator by product of norms
    norm_dot_product = numerator / (norm_i * norm_j)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    sim_mat = torch.matmul(out_norm, out_norm.T)
    sim_mat_c = torch.exp(sim_mat/tau)
    sim_mat_diag0 = sim_mat_c.fill_diagonal_(0)
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        denominator_1 = sim_mat_diag0[k].sum()
        denominator_2 = sim_mat_diag0[k+N].sum()
        l1 = - torch.log(numerator / denominator_1)
        l2 = - torch.log(numerator / denominator_2)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_posi_pairs = sim_positive_pairs(out[:N],out[N:]) # (N,1)
    ...
    sim_posi_pairs = sim_posi_pairs.view(-1)  # [N]
    num = torch.exp(sim_posi_pairs / tau)  # [N]
    numerator = torch.cat([num, num], dim=0)  # [2*N]
    ...
    loss_vec = - torch.log(numerator/denom)
    loss = loss_vec.sum() / (2*N)
